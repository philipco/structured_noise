...
if __name__ == '__main__':
    ...
    vanilla_sgd = SGDVanilla(clients, nb_epoch=NB_EPOCH, sto=STOCHASTIC, batch_size=BATCH_SIZE)
    sgd_nocompr = vanilla_sgd.gradient_descent(label=labels[0], deacreasing_step_size=DECR_STEP_SIZE)

    my_compressors = [synthetic_dataset.quantizator, synthetic_dataset.sparsificator, #synthetic_dataset.all_or_nothinger]
    ...
    for i in range(len(my_compressors)):
        ...
        all_sgd.append(
            SGDCompressed(clients, compressor, nb_epoch=NB_EPOCH, sto=STOCHASTIC, batch_size=BATCH_SIZE).gradient_descent(
            label=labels[i+1], deacreasing_step_size=DECR_STEP_SIZE))
        # all_sgd.append(
        #     SGDArtemis(clients, compressor, nb_epoch=NB_EPOCH, sto=STOCHASTIC, batch_size=BATCH_SIZE).gradient_descent(
